...
class ServerStorage:
    ...
    def add_contact(self, user_name, contact_name):
        # Query users IDs from users table
        user_id = self.session.query(self.User).filter_by(name=user_name).first().id
        contact = self.session.query(self.User).filter_by(name=contact_name)
        if contact.count():
            contact_id = contact.first().id 
            if not self.session.query(self.Contacts).filter_by(user_id=user_id, contact_id=contact_id).count():               
    ...
    def get_contacts(self, user_name):
        user_id = self.session.query(self.User).filter_by(name=user_name).first().id
        contacts = self.session.query(self.Contacts).filter_by(user_id=user_id).all()
        contacts_list = []
    ...
    def get_users_history(self):
        ...
        users_list = []
        for user in users:
            users_list.append(f'User#{user.user_id} logged in at {user.login_time}')
        return users_list
